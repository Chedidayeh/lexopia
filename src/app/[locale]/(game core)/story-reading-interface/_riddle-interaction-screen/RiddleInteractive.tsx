...
interface RiddleInteractiveProps {
  ...
  resolvedAttempt?: ChallengeAttempt;
}
...
type FeedbackState = {
  type: "solved" | "almost" | "skipped" | "incorrect" | null;
  message: string;
  isVisible: boolean;
};

function getSolvedMessageForType(
  type: ChallengeType,
...
function createInitialFeedbackState(
  resolvedAttempt: ChallengeAttempt | undefined,
  challengeType: ChallengeType,
  t: ReturnType<typeof useTranslations>,
): FeedbackState {
  if (resolvedAttempt?.status === ChallengeStatus.SOLVED) {
    return {
      type: "solved",
      message: getSolvedMessageForType(challengeType, t),
      isVisible: true,
    };
  }
  if (resolvedAttempt?.status === ChallengeStatus.SKIPPED) {
    return {
      type: "skipped",
      message: t("feedbackDisplay.skipButton"),
      isVisible: true,
    };
  }
  return { type: null, message: "", isVisible: false };
}
...
const RiddleInteractive = ({
  ...
  const [attempts, setAttempts] = useState(initialAttemptCount);
  const [hintsUsed, setHintsUsed] = useState(initialHintsUsed);
  const [currentHintLevel, setCurrentHintLevel] = useState(initialHintsUsed);
  const [isHintPanelVisible, setIsHintPanelVisible] = useState(false);
  const [feedbackState, setFeedbackState] = useState<FeedbackState>(() =>
    createInitialFeedbackState(resolvedAttempt, viewModel.type, t),
  );
  const [isPlayingAudio, setIsPlayingAudio] = useState(false);
  const [elapsedTime, setElapsedTime] = useState(
    resolvedAttempt?.timeSpentSeconds ?? 0,
  );
  const [isTimerRunning, setIsTimerRunning] = useState(
    () =>
      resolvedAttempt?.status !== ChallengeStatus.SOLVED &&
      resolvedAttempt?.status !== ChallengeStatus.SKIPPED,
  );
  ...
  const getSolvedMessage = useCallback(
    (type: ChallengeType) => getSolvedMessageForType(type, t),
    [t],
  );
  ...
  const handleContinue = (action: "solved" | "skipped") => {
    ...
    if (!alreadyTerminal && action === "skipped") {
      const localAttempt = createChallengeAttempt({
        challengeId: viewModel.id,
  ...
  return (
    <div className="pt-16 md:pt-0 sm:pt-20 pb-20 sm:pb-24 md:pb-28 lg:pb-32">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 max-w-5xl">
        ...
        <FeedbackDisplay
          type={
            feedbackState.type === "skipped" ? "solved" : feedbackState.type
          }
